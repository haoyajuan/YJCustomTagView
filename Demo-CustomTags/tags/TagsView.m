...
- (void)setTagsFrame:(TagsViewFrame *)tagsFrame
{
    ...
    for (NSInteger i=0; i<tagsFrame.tagsArray.count; i++) {
        ...
        [tagsButton setTitle:tagsFrame.tagsArray[i] forState:UIControlStateNormal];
        [tagsButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        tagsButton.titleLabel.font = TagsTitleFont;
        tagsButton.backgroundColor = [UIColor whiteColor];
        tagsButton.layer.borderWidth = 1;
        tagsButton.layer.borderColor = [UIColor lightGrayColor].CGColor;
        tagsButton.layer.cornerRadius = 4;
        tagsButton.layer.masksToBounds = YES;
        
        tagsButton.frame = CGRectFromString(tagsFrame.tagsFrames[i]);
